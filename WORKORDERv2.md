...
1. **08:00 CET/CEST — Morning Macro + Asia Pulse**
   ...
   - Scan key news sources: geo-political developments, major financial news, earnings calendar, commodities futures (oil, gas, gold, silver), crypto.
   ...
   - Apply this view to the Nordic, UK and EU Watchlist (skip exchanges with bank holiday) + current Portfolio (exclude US).
...
2. **≈ 2 hours before Nordic/EU close — Pre-EU/Nordic Close (US Open Focus)**
   ...
   - Re-run sentiment scoring on US Watchlist + Portfolio with fresh US-open lens, unless bank holiday.
   - Generate actionable trade suggestions (buy/sell/rebalance) that respect all hard portfolio rules.

3. **≈ 1 hour before US close — Pre-US Close Final Assessment -- Unless bank holiday **
...
- We do not have real time prices, when fetching price data from the Saxo platform the `DelayedByMinutes` field in the price response (Quote object) will show 15 when you are on the default/delayed feed, keep this in mind when placing orders (maybe apply limits to orders and then as we fetch new prices ecery minute check if the orders are fullfilled or make adjustments).
